...
def randomly_gen_uspace_url():
    ...
    for i in range(12):
        ...
        url = "https://www.zylsj.com/space-uid-{}.html".format(str(uid))
        url_list.append(url)
    return url_list
...
def login(username, password):
    ...
    login_url = "https://www.zylsj.com/member.php?mod=logging&action=login&loginsubmit=yes&infloat=yes&lssubmit=yes&inajax=1"
    login_data = {
...
def check_login_status(s, number_c):
    test_url = "https://www.zylsj.com/home.php?mod=spacecp"
    res = s.get(test_url)
    res.encoding = "utf-8"
    test_title = re.findall("<title>.*?</title>", res.text)
    if test_title[0] != "<title>个人资料 -  资源来收集 -  Powered by Discuz!</title>":
